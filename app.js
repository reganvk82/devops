...
var log = function(severity, entry) {
    ...
    if (severity === SEVERITY_DEBUG) {
          if (LOG_MODE == SEVERITY_DEBUG) {
            // dont log debug unless Log_mode is Debug
            var logEntry = new Date().toISOString() + ' - ' + severity + " - " + LOG_MODE + " - " + entry + '\n';
...
var server = http.createServer(function (req, res) {
    if (req.method === 'POST') {
        ...
        if(url.pathname === "/api/changeLogMode") {
            // Usage: /api/changeLogMode?mode=Debug
            var newMode = url.query["mode"];
            if ( newMode == null) {
                status = "Usage: /api/changeLogMode?mode=Debug"
            } else {
                log(SEVERITY_INFO, "Attempt to change Log Mode from " + LOG_MODE + " To " + newMode);
                switch(newMode) {
                    case SEVERITY_DEBUG:
                    case SEVERITY_ERROR:
                    case SEVERITY_INFO:
                    case SEVERITY_WARNING:
                        if ( LOG_MODE == newMode) {
                            status = "Log Mode is already " + newMode;
                        }else {
                            log(SEVERITY_WARNING, "Somebody is changing! Log Mode from " + LOG_MODE + " to " + newMode);
                            LOG_MODE = newMode;
                            status = "Log Mode is changed to " + newMode;
                        }
                        break;
                    default:
                        log(SEVERITY_ERROR, "Invalid Log Mode : " + newMode);
                        status = "Invalid Log Mode : " + newMode + "\n";
                        status = status + "Validate Log Modes are [" + SEVERITY_DEBUG + "|" + SEVERITY_ERROR + "|" + SEVERITY_INFO + "|" + SEVERITY_WARNING + "]" ;
                }
            }
        }
